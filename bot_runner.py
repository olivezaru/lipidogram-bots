...
# Официальные ID каналов YouTube для прямого доступа через RSS без блокировок
GLOBAL_HEALTH_CHANNELS = [
    {"name": "Dr. Peter Attia (эксперт по долголетию и липидологии)", "channel_id": "UCF_fDSgblvyC-hltP1t4gTg"},
    {"name": "Huberman Lab (нейробиолог Стэнфордского университета)", "channel_id": "UC2D2CMWXMOVWx7giW1n3LIg"},
    ...
    {"name": "Dr. Brad Stanfield (превентивная кардиология)", "channel_id": "UCpcvPevmCfu_UhyvCc_1K8A"},
    {"name": "Simon Hill / The Proof (исследования питания и липидов)", "channel_id": "UCE0f85hX8Qz2e0n_W1i5qKw"},
    {"name": "Доктор Утин (кардиохирург Алексей Утин)", "channel_id": "UCe1Qc_VqL_8rLq9a-tM_HwQ"},
    {"name": "СМТ — Научный подход (Борис Цацулин)", "channel_id": "UCi1p7P6-O3sV3h98rW2g6mA"}
]
...
# НАСТОЯЩИЙ ПАРСИНГ ВИДЕО YOUTUBE ЧЕРЕЗ RSS БЕЗ БЛОКИРОВОК И КАПЧИ
def fetch_global_youtube_video() -> dict:
        ...
        try:
            ...
            if req.status_code == 200:
                ...
                for entry in entries[:6]:
                    ...
                    # Пытаемся получить субтитры конкретного ролика
                    transcript_text = ""
                    try:
    ...
    # Если совсем не удалось достучаться до YouTube, берем актуальную статью из PubMed
...
if __name__ == "__main__":
    asyncio.run(main())    {"name": "Huberman Lab (Стэнфорд, США)", "handle": "@hubermanlab"},
